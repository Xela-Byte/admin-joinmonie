...
import { allCampaignRoute, host } from "../utils/APIRoutes";
...
const BlockCampaigns = () => {
  ...
  const blockCampaign = async () => {
    ...
    const reportConfig = {
      method: "POST",
      url: `${host}/admin/campaigns/${campaignId}/report`,
      headers: headers,
      data: data,
    };
  ...
  return (
    <DonationContainer>
      ...
      <DonationInputContainer>
        <label htmlFor="Campaign ID">Please state reason for blocking</label>
        <textarea
          type="text"
          name="campaignId"
          ...
          rows={5}
        />
        <button onClick={() => blockCampaign()}>Block</button>
      </DonationInputContainer>
      <DonationCampaignContainer>
        <p
